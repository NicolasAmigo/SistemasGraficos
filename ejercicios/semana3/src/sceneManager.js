...
export class SceneManager {
	...
	construirVehiculo() {
		...
		//var llanta_clone = this.llanta.clone();
		//this.scene.add(llanta_clone);
		//llanta_clone.position.set(0, 0, 0);

		this._crearParteSuperior();
		this._crearParteInferior();
		this.chasis.add(this.cabina);
		this.cabina.position.set(0,25,0);
		this.chasis.position.set(0, 0, 0);
		this.vehiculo.add(this.chasis);

		this.llanta.visible = false
		this.eje.visible = false
		this.tuerca.visible = false
		this.cubierta.visible = false
	...
	_crearRueda() {
		...
		for (let i = 0; i < 8; i++) {
			let tuerca = this.tuerca.clone();
			rueda.add(tuerca);
			tuerca.position.set(radio_tuerca * Math.cos(i * Math.PI/4),
			radio_tuerca * Math.sin(i * Math.PI/4),
				3);
		}
		return rueda
	}
	...
	_crearEjeRuedas() {
		let ejeRuedas = new THREE.Group();
		let eje = this.eje.clone();
		let rueda1 = this._crearRueda();
		let rueda2 = this._crearRueda();

		ejeRuedas.add(eje);
		eje.position.set(0,0,0)
		ejeRuedas.add(rueda1);
		rueda1.position.set(0, 0, 27);
		ejeRuedas.add(rueda2);
		rueda2.position.set(0, 0, -27);
		rueda2.rotateY(Math.PI)

		ejeRuedas.position.set(0, 0, 0)

		return ejeRuedas
	}

	_crearParteSuperior() {
		this.cabina.position.set(0,0,0);
	...
	_crearParteInferior() {
		let eje1 = this._crearEjeRuedas();
		let eje2 = this._crearEjeRuedas();

		this.chasis.add(eje1);
		eje1.position.set(20,5,0);
		this.chasis.add(eje2);
