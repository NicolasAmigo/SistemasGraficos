...
export class SolarSystem {
	...
	setupSceneGraph() {
		...
		let earthOrbit = this.earthOrbit;
		let issOrbit = this.issOrbit;
		let moonOrbit = this.moonOrbit;

		var earthAxis = new THREE.Group();

		sun.add(earthOrbit);
		earthOrbit.add(moonOrbit);
		moonOrbit.add(moon);
		earthOrbit.add(earthAxis);
		earthAxis.add(earth)
		earth.add(issOrbit);
		issOrbit.add(iss);
		moon.add(apollo);

		earthOrbit.position.set(300, 0, 0);
		earth.position.set(0, 0, 0)
		earthAxis.rotateX(0.401426);
		apollo.position.set(0,10,0);
		moon.position.set(100, 0 ,0);
		iss.position.set(0, 40, 0);
		// angulo raro
		iss.rotateX(Math.PI/6* 11);
	...
	update(time) {
		...
		this.earthOrbit.position.set(earth_radius * Math.cos(2*Math.PI * time / year), 
						0, 
						earth_radius * Math.sin(2*Math.PI * time / year))
		
		earth.rotation.y = time * 2 * Math.PI / year * 10
		this.issOrbit.rotation.x = 2 * Math.PI * time / year * 5;
		this.moonOrbit.rotation.y = 2 * Math.PI * time / year * 5;
